...
static boolean ENABLE_SERIAL = true;
...
void setup() {
  ...
  background(150);
   ...
   setup renderers
  vortex = new VortexRenderer(current_player);
  radar = new RadarRenderer(current_player);
  iso = new IsometricRenderer(current_player);
   
  visuals = new AudioRenderer[] {radar, vortex,  iso};
   
  // activate first renderer in list
  visualizer_select = 0;
  current_player.addListener(visuals[visualizer_select]);
  visuals[visualizer_select].setup();
  
  thread("poll_for_changes");
}
...
void poll_for_changes() {
  ...
  dataObject.put("music_player_genre","hangout");  // Soothing
  dataObject.put("music_player_enable",true);
...
void draw() {
  poll_for_changes();
  visuals[visualizer_select].draw();
...
//    current_player.removeListener(visuals[visualizer_select]);
    music_player_enable = 0;
